...
class Mentor extends NeonAcademyMember {
  ...
  Mentor({
    ...
    required super.motivationLevel,
    required super.team,
  });
}
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void _runPart2Challenges() {
    ...
    void printTeamMotiviation(Team team) {
      switch (team) {
        ...
        case Team.iosDevelopmentTeam:
          print(
            'iOS Geliştirme Ekibi: iOS geliştirme konusundaki uzmanlığınız, her uygulamanızı Apple cihazlarda mükemmel, sezgisel ve sorunsuz bir kullanıcı deneyimi sunmasını sağlar.',
          );
          break;
        case Team.androidDevelopmentTeam:
          print(
  ...
  void _runPart3Challenges() {
    ...
    // 2. Görev: Motivasyon Seviyesine Göre String Döndürme
    String getMotivationStatus(int? level) {
      if (level == null || level == 0) return 'Hiç motivate değil';
      if (level > 5) return 'Çok motive';
      return 'Orta düzeyde motive'; // 1 ile 5 arası
    }

    // 3. Görev: Nil Coalescing (?? operatörü)
    int getSafeMotivationLevel(NeonAcademyMember member) {
      return member.motivationLevel ?? 0;
    }

    // 4. Görev: if let Karşılığı (Null Check) ve Hedef Karşılaştırma
    bool isTargetReached(NeonAcademyMember member, int target) {
      final level = member.motivationLevel;
      if (level != null) {
        return level >= target;
      }
    ...
    testMember.increaseMotivation(3); // Null olduğu için 1 yapacak
    print('Durum: ${getMotivationStatus(testMember.motivationLevel)}');

    print('\n--- Motivasyonu Artır (İkinci Kez) ---');
    testMember.increaseMotivation(6); // 1 + 6 = 7 olacak
    printMotivationMessage(testMember);
